...
scp -i $EC2_PRIVATE_KEY -r -P $PORT \
  $COOKBOOK_TARBALL \
  $DNA \
  $USERNAME@$IP:.
...
eval "ssh -q -t -p \"$PORT\" -l \"$USERNAME\" -i \"$EC2_PRIVATE_KEY\" $USERNAME@$IP \"sudo -i which chef-solo > /dev/null \""
...
#Okay, run it.
eval "ssh -t -p \"$PORT\" -l \"$USERNAME\" -i \"$EC2_PRIVATE_KEY\" $USERNAME@$IP \"sudo -i sh -c 'cd $CHEF_FILE_CACHE_PATH && \
